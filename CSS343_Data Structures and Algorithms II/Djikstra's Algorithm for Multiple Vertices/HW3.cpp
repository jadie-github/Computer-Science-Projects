...
// perform Dijkstra's algorithm for CSS 343 Program 3.
...
int main() {
    ...
    for (;;) {
        ...
        cout << "numedge is " << G.numEdge() << endl;
        Graph A(G);
        G = A = G = A;
        if (infile1.eof())
            break;
        A.findShortestPath();
        A.displayAll();              // display shortest distance, path to cout
        cout << endl << "The path from vertex 2 to vertex 3 is:" << endl;
        A.display(2, 3);             // display path from node 3 to 1 to cout
        cout << endl;
